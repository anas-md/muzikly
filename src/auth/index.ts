...
class Auth {
  ...
  async getCurrentUser(
    option: { withSession: boolean } | undefined = undefined
  ): Promise<any> {
    const session_id = cookies().get(this.session_cookie_name);
    if (!session_id || !validate(session_id.value))
      return { error: 'Invalid Session ID' };
    try {
      await this.dbconnect();
      const session = await Session.findById(session_id.value, {
        __v: false,
      }).populate('user', {
        __v: false,
      });
  ...
  async deleteSession(filter: { _id?: string; user?: string }) {
    ...
    try {
      ...
      if (!deletedSessions.deletedCount)
        return {
          success: 'Session was not found with provied Session/User ID',
        };
      return {
        success: filter.user
          ? 'Deleted sessions by user id'
          : 'Deleted session by session id',
      };
    } catch (error) {
