  ...
  return (
    <form ref={form} onSubmit={Search} className="relative flex w-full gap-2">
      ...
      <Input
        ...
        className="max-w-sm rounded-lg pl-10 focus-visible:ring-indigo-500 dark:text-slate-200 dark:focus-visible:ring-indigo-500"
        value={input}
            ...
            >
              ...
              >
                {currentSong.id === song.id && isAudioPlaying ? (
                  <LuPauseCircle className="size-7" />
                ) : (
                  <FaRegCirclePlay className="size-[25px]" />
                )}
              </div>
            </div>
          ))}
