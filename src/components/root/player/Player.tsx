...
export default function Player() {
  ...
  const { currentSong, Playing, volume, shuffle, isAudioPlaying } =
    useSnapshot(playerStore);

  // Formats song Duration
  const duration = useMemo(() => {
    const time = Duration.fromObject({
      second: Number(currentSong.duration) || 0,
    }).toFormat('mm:ss');
    return time;
  }, [currentSong]);
      ...
      <div className="flex max-w-lg basis-full flex-col gap-2 lg:max-w-xl lg:flex-row lg:gap-6 xl:max-w-2xl">
        ...
        <div className="flex justify-between">
          ...
          <div className="flex items-center gap-2 text-2xl text-black/90 dark:text-white/90 md:gap-3">
            ...
            <MdSkipNext
              onClick={() => playerStore.playNextSong()}
              className="cursor-pointer"
            />
          ...
          <div className="flex grow basis-1/6 items-center justify-end gap-2 lg:hidden">
            <div
              ...
              onClick={() => playerStore.toggleVolume()}
            >
